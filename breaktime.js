...
export class BreakTime {
  ...
  static async startBreak() {
    if (setting("paused")) BreakTime.showApp();
    else {
      ...
      if (setting("auto-pause")) game.togglePause(true, true);
    }
  }

  static async endBreak() {
    if (setting("auto-pause") && game.paused) game.togglePause(false, true);
  ...
  static async showApp(playSound = true) {
    ...
    } else BreakTime.app.render(true);
  ...
  static async changeReturned(data) {
    if (game.user.isGM) {
      ...
      let state =
        data.state != undefined ? data.state : !setting("break")[userId];
  ...
  static getRandomText(text) {
    ...
    let idx = Math.clamp(
      parseInt(Math.random() * parts.length),
      0,
      parts.length - 1
    );
    return parts[idx];
  ...
  static async adjustStatus(data = {}) {
    ...
    if (userId == game.user.id) {
      let message =
        data.message ||
        (data.away
          ? BreakTime.getRandomText(i18n(setting("away-text")))
          : BreakTime.getRandomText(i18n(setting("back-text"))));
      ...
      if (["both", "chat"].includes(setting("notify-option"))) {
        const speaker = setting("chat-bubble")
          ? {
        ...
        let tkn = canvas.scene.tokens.find(
  ...
  static refresh() {
    if (BreakTime.app && BreakTime.app.rendered) BreakTime.app.render();
  ...
  static refreshToolbar(data) {
    if (game.user.id == data.userId) {
      ...
      let tool = ui.controls.controls
        .find((c) => c.name == "token")
        ?.tools.find((t) => t.name == "togglebreak");
...
Hooks.on("getSceneControlButtons", (controls) => {
  ...
  tokenControls.tools.push({
    name: "togglebreak",
    title: awayData.includes(game.user.id)
      ? "BREAKTIME.button.return"
      : "BREAKTIME.button.title",
    icon: "fas fa-mug-hot",
    onClick: (away, event) => {
...
Hooks.on("renderPlayerList", async (playerList, html, data) => {
  ...
  setting("away").forEach((userId) => {
    ...
    html
      .find(`[data-user-id="${userId}"] .player-active`)
      .css({ background: "transparent" });
  });

  if (
    setting("show-button") &&
    (game.user.isGM || (!game.user.isGM && setting("paused"))) &&
    $(".breaktime-button", html).length === 0
  ) {
    $("<button>")
      .addClass("breaktime-button minimal-ui-button")
      .append(`<i class="fas fa-coffee"></i>`)
      .insertAfter($("ol", html))
      .click(
        game.user.isGM
          ? BreakTime.startBreak.bind()
          : BreakTime.showApp.bind(this, false)
      );
  }
});
...
Hooks.on("chatCommandsReady", function (commands) {
  if (commands.register != undefined) {
    ...
    commands.registerCommand(
      commands.createCommandFromData({
        commandKey: "/brb",
        invokeOnCommand: (chatlog, messageText, chatdata) => {
          BreakTime.emit("adjustStatus", {
            away: true,
            message: messageText || setting("away-text"),
          });
        },
        shouldDisplayToChat: false,
    ...
    commands.registerCommand(
      commands.createCommandFromData({
        ...
        invokeOnCommand: (chatlog, messageText, chatdata) => {
          BreakTime.emit("adjustStatus", {
            away: false,
            message: messageText || setting("back-text"),
          });
        },
        shouldDisplayToChat: false,
...
Hooks.on("globalInterfaceVolumeChanged", (volume) => {
  if (!game.modules.get("monks-sound-enhancements")?.active) {
    ...
    if (BreakTime.endSound) {
      BreakTime.endSound.volume =
        (BreakTime.endSound.effectiveVolume ?? 1) * volume;
    }
  }
});
...
Hooks.on("renderSettingsConfig", (app, html, data) => {
  $("select[name='breaktime.break-playlist']", html)
    .empty()
